...
discrete_record_np_cache_size: int = 8_192
discrete_record_histogram_steps: int = 128
...
class RecordDiscrete:
    ...
    def record(self, value: float) -> None:
        ...
        if discrete_record_histogram_steps > 0:
            index: int = round(value) // self.__histogram_stepwide
            b: int = len(self.__histogram)
            a: int = int(b / 2)
            while index >= b:
    ...
    def sd(self) -> float:
        ...
        var: float = self.__sum2 / (self.__count - 1) - (self.__sum**2) / self.__count / (self.__count - 1)
...
class RecordContinuous:
    ...
    def __init__(self, record_values: bool = False) -> None:
        ...
        """
        self.__lastTime: float = -1
        self.__lastValue: float = -1
        self.__timeSum: float = 0
        self.__valueSum: float = 0
        self.__min: float = 0
        self.__max: float = 0
        self.__recordedTimestamps: Optional[list[float]] = None
        self.__recordedValues: Optional[list[float]] = None
        if record_values:
    ...
    def record(self, time: float, value: float) -> None:
        ...
        if self.__lastTime >= 0:
            delta: float = time - self.__lastTime
            last: float = self.__lastValue
            if delta > 0:
...
class RecordOptions:
    ...
    def record(self, option: Any) -> None:
